...
public class ImageLoaderWithDoubleCaches {
    ...
    private Set<ASyncDownloadImage3> mTasks;
    ...
    public void loadImages(int start, int end){
        for (int i = start; i < end; i++){
            ...
            Bitmap bitmap = getBitmapFromMemoryCaches(url);
            if (bitmap == null){
                ASyncDownloadImage3 task = new ASyncDownloadImage3(url);
                mTasks.add(task);
                task.execute(url);
            } else {
    ...
    public void cancelAllTasks(){
        if (mTasks != null){
            for (ASyncDownloadImage3 task : mTasks){
                task.cancel(false);
            }
        }
    }
    ...
    class ASyncDownloadImage3 extends AsyncTask<String, Void, Bitmap>{
        private String url;

        public ASyncDownloadImage3(String url){
            this.url = url;
        }
