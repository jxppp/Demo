package com.example.administrator.listviewandviewpage.view;
...
public class SmartViewPager extends ViewPager {
    ...
    private float getViewPagerRatio(Context context, AttributeSet attrs, int[] styleableIds){
        TypedArray attributes = context.obtainStyledAttributes(attrs, styleableIds);
        float ratio = 0.0f;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (wMode == MeasureSpec.EXACTLY && hMode != MeasureSpec.EXACTLY && ratio != 0.0f){
            hSize =(int) (wSize / ratio + 0.5f);
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(hSize, MeasureSpec.EXACTLY);
        }else if (hMode == MeasureSpec.EXACTLY && wMode != MeasureSpec.EXACTLY && ratio != 0.0f){
