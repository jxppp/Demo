...
public class SlideCutListView extends ListView {
    ...
    //用来指示item滑出屏幕的方向，向左或向右，用一个枚举值标记
    ...
    public SlideCutListView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTouchSloop = ViewConfiguration.get(getContext()).getScaledTouchSlop();
    }
        ...
        switch (ev.getAction()){
            ...
            case MotionEvent.ACTION_MOVE:
                if(Math.abs(getScrollVelocity()) > SNAP_VELOCITY
                        || (Math.abs(ev.getX() - downX) > mTouchSloop
                            && Math.abs(ev.getY() - downY) < mTouchSloop)){
                    isSlide = true;
                }
                break;
    ...
    private void scrollRight(){
        ...
        //调用startScroll方法来设置一些滚动的参数，我们在computeScroll（）方法中调用scrllTo来滚动item
